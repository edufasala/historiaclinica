<?php
	...
	foreach ($cn['hcpadodonto'] as $fila)
	{
		...
		$ecreador=utf8_decode($fila->evolucion_created_user);
	...
	$pdf->Ln(3);	
	// $pdf->Ln($esp2);
	$pdf->Cell(45,5,strtoupper('ANTECEDENTES PERSONALES'),0,0,'',1);
	$pdf->Ln($esp);
	//$pdf->MultiCell(190,5,'Tratamiento Médico: '.$atratamiento,0,'L');
	$pdf->Cell(95,5,'Tratamiento Medico: '.$etmedico,0,0,1);
	$pdf->Cell(95,5,'Ingestion de Alimentos: '.$ealimentos,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Reacciones Alergicas: '.$ealergicos,0,0,1);
	$pdf->Cell(95,5,'Anestesia: '.$eanestecia,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Antibioticos: '.$eantibiotico,0,0,1);
	$pdf->Cell(95,5,'Hemorragias: '.$ehemorragias,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Radiaciones: '.$eirradiaciones,0,0,1);
	$pdf->Cell(95,5,'Enfermedad Respiratoria: '.$eenfrespiratoria,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Cardiopatias: '.$ecardiopatias,0,0,1);
	$pdf->Cell(95,5,'Artritis: '.$eartritis,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Hepatitis: '.$ehepatitis,0,0,1);
	$pdf->Cell(95,5,'Embarazo: '.$eembarazo,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Enfermedades Renales: '.$erenal,0,0,1);
	$pdf->Cell(95,5,'GastroIntestinales: '.$egastro,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Antecedentes Quirugicos: '.$equirurgico,0,0,1);
	$pdf->Cell(95,5,'Osteoporosis: '.$eosteo,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Otros Motivos: '.$eotros,0,0,1);
	$pdf->Ln($esp);
	$pdf->Ln(2);
	$pdf->Cell(30,5,strtoupper('OBSERVACIONES'),0,0,'',1);
	$pdf->Ln($esp);
	$pdf->MultiCell(190,5,$eobserva,0,'L');
	$pdf->Ln($esp);
	$pdf->SetX(10);
	$pdf->Cell(42,5,strtoupper('HABITOS ODONDOLOGICOS'),0,0,'',1);
	$pdf->Ln($esp);
	$pdf->Cell(95,5,'Se realiza Cepillado: '.$ecepillado,0,0,1);
	$pdf->Cell(95,5,'Usa Crema Dental: '.$ecrema,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Habitos Normales: '.$enormal,0,0,1);
	$pdf->Ln($esp);	
	$pdf->Ln(2);	
	$pdf->Cell(45,5,strtoupper('GRADO DE CARIOGENICIDAD'),0,0,'',1);
	$pdf->Ln($esp);	
	$pdf->Cell(95,5,'Toma Bebidas Azucaradas: '.$ebebida,0,0,1);
	$pdf->Cell(95,5,'Toma Masas No Azucaradas: '.$emasas,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Come Caramelos: '.$ecaramelo,0,0,1);
	$pdf->Cell(95,5,'Toma Masas No Azucaradas: '.$emasas,0,0,1);
	$pdf->Ln(4,5);	
	$pdf->Cell(95,5,'Toma Azucar: '.$eazucar,0,0,1);
	$pdf->Cell(95,5,'Bebe Alcohol Frecuentemente: '.$ealcohol,0,0,1);
	$pdf->Ln(4,5);
	$pdf->Cell(95,5,'Fuma: '.$efuma,0,0,1);
	$pdf->Ln(7);
	$pdf->Ln($esp);	
	$pdf->SetFont('Arial','',8);
