<?php
	...
	foreach ($cn['hcpadenf'] as $fila)
	{
		...
		$cuidadopfeiffer=utf8_decode($fila->enf_pad_pfeiffer);
		$cuidadobarthel=utf8_decode($fila->enf_pad_barthel);
		$cuidadonorton=utf8_decode($fila->enf_pad_norton);
		$cuidadobraden=utf8_decode($fila->enf_pad_bradenq);
		$cuidadokarnosky=utf8_decode($fila->enf_pad_karnosky);
	...
	else
	{
		...
		foreach ($cn['hcvaloracionenf'] as $fila)
		{
			...
			$auxareadetalle=$fila->enf_valoracion_vivienda_detalle;
	...
	class PDF extends FPDF
	{
		...
		public function Header()
		{
			...
			// $this->Cell(65); 'Enf. '.$GLOBALS['nomenfermera']
			...
			$this->SetX(9);
			$this->SetFont('Arial','',8);
			$this->Cell(198,4, strtoupper('HISTORIA CLINICA DE ENFERMERA GENERAL'),0,0,'C',true);
			$this->Ln(4);
			$this->SetX(9);
	...
	$pdf->Ln(5);
	$pdf->Cell(196,5,utf8_decode($dom1p1),0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,utf8_decode('¿Nesecita mayor informacion sobre su enfermedad?'),0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,utf8_decode($dom1p2),0,0,1);
	$pdf->Ln(6);
	$pdf->Cell(196,5,strtoupper('Dominio 2: Nutricion'),0,0,'C',1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,utf8_decode('Dieta: ').$dom2nutricion.'   Otros: '.$dom2otro,0,0,1);
	$pdf->Ln(5);
	...
	$pdf->Ln(5);
	$pdf->Cell(196,5,$dom3vomito.'   Caracteristicas: '.$d3caracteristicas,0,0,1);
	$pdf->Ln(5);
	...
	$pdf->Ln(5);
	$pdf->Cell(196,5,utf8_decode('Sueño: ').$dom4sueno.'   Otros: '.$d4otros,0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,'Fiebre: '.$dom4fiebre.'   Tipo: '.$d4tipo,0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,'Movilidad Fisica: '.$dom4movilidad.'   : '.$d4movilidadetalle,0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,'Apoyos Externos: ',0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,'Fuerza Muscular: '.$d4musculardetalle,0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,'Respuestas Cardiovasculares Respiratorias: ',0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,'FC: '.$dom4respiratorias.'   llenado Capilar: '.$d4capilar,0,0,1);
	$pdf->Ln(5);
	$pdf->Cell(196,5,'Pulso: '.$dom4pulso.'   Fecha implamtacion: '.$d4pulsofecha,0,0,1);
	$pdf->Ln(5);
	...
	$pdf->Ln(6);
	$pdf->Cell(196,5,strtoupper('Determinacion nivel de cuidado de enfermera'),0,0,'C',1);
	$pdf->Ln(5);
	...
	if($ellargo==0)
	{
		$pdf->MultiCell(196,5,$obldig,0,'L');
		$pdf->Ln(6);
		$unsalto=4;
	}
	else
	...
	if($imprimir2=="0")
	{
		// $pdf->Cell(150,5,'no',0,0,1);
	}
	else
	{
		// $pdf->Cell(150,5,'si : ',0,0,1);
		// $pdf->Cell(150,5,'si : '.var_dump($cn['hcvaloracionenf']),0,0,1);
		$pdf->Ln(5);
		$pdf->SetFont('Arial','',8);
		...
		$pdf->Ln(6);
		$pdf->Cell(196,5,'Situacion Familiar   Puntos: '.$situacion_familiar,0,0,1);
		$pdf->Ln(4);
		...
		$pdf->Cell(38,4,utf8_decode('vive con conyuge de similar'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('vive con familia y/o conyuge'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('vive solo y tiene hijos'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('vive solo y carace de hijos o'),0,0,'J');
		$pdf->Ln(4);
		$pdf->Cell(38,4,utf8_decode('dependencia fisico psiquica'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('edad'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('y presenta algun grado de'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('proximos'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('viven alejados'),0,0,'J');
		$pdf->Ln(4);
		...
		$pdf->Cell(38,4,utf8_decode('Desde el salario minimo a'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('Pension no contributiva,'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('Sin ingresos o ingresos de'),0,0,'J');
		$pdf->Ln(4);
		$pdf->Cell(38,4,utf8_decode('minimo'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('minimo hasta el salario'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('pension minima contributiva'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('ingreso subsidiado'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('apoyo familiar'),0,0,'J');
		$pdf->Ln(4);
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('minimo incluso'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Ln(6);
		...
		$pdf->Cell(38,4,utf8_decode('Barreras arquitectonicas en'),0,0,'J');
		...
		$pdf->Cell(38,4,utf8_decode('habitacion integral,'),0,0,'J');
		$pdf->Ln(4);
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('casa (peldaños, escalones,'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('(sin baño completo, sin'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('comunicacion audio visual)'),0,0,'J');
		...
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('metalica o papel duro, sin'),0,0,'J');
		$pdf->Ln(4);
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		...
		$pdf->Cell(38,4,utf8_decode('Relacion social solo con'),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('Relacion social solo con'),0,0,'J');
		...
		$pdf->Cell(38,4,utf8_decode(''),0,0,'J');
		$pdf->Cell(38,4,utf8_decode('residencia geriatrica'),0,0,'J');
