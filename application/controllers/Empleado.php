<?php
	...
	class Empleado extends CI_Controller {
		...
		function index(){
			...
			$recibodatos['cn']=$this->empleado_model->consultar_perfil($this->session->userdata('s_id'));
			$this->load->view('plantilla/header');
			$this->load->view('plantilla/menu_e');
			$this->load->view('perfil/vempleado',$recibodatos);
		...
		function actualizar(){
			...
			$rs=$this->empleado_model->actualizar_perfil($datos);
			echo $rs;
		}
		function calcular(){
			$dato=$this->input->post('lafecha');
			$rs=$this->empleado_model->calcular_edad($dato);
			echo $rs;			
		}
