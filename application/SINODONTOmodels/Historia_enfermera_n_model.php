<?php
	...
	class Historia_enfermera_n_model extends CI_Model
	{
		...
		function registrar_historiapad_enf($datos){
			...
			if($result>=1)
			{
				$existe2="SELECT COUNT(*) hc_enf_pad,enf_pad_f_reg,MAX(enf_pad_h_reg) enf_pad_h_reg FROM hc_enf_pad WHERE enf_pad_doc='".$datos['enf_pad_doc']."' and enf_pad_f_reg='".$getfecha."' ";
				$resultado2=$this->db->query($existe2);
				$resultado2=$resultado2->row();
				$result2=$resultado2->hc_enf_pad;
				$result2f=$resultado2->enf_pad_h_reg;
		...
		function actualizar_historiapad_enf($datos){
			...
			$this->db->where("enf_pad_h_inicio",$datos['enf_pad_h_inicio']);
		...
		function registrar_historiintervencion_enf($datos){
			...
			$getfecha=date("Y-m-d");
			$gethora=date("h:i:s");
			$pvalidar="SELECT COUNT(*) hc_enf_intervencion FROM hc_enf_intervencion WHERE enf_intervencion_doc='".$datos['enf_intervencion_doc']."' ";
			$pvalidar=$this->db->query($pvalidar);
			$pvalidar=$pvalidar->row();
			$pvalidar=$pvalidar->hc_enf_intervencion;
			if($pvalidar>=1)
			{
				$existe="SELECT COUNT(*) hc_enf_intervencion,enf_intervencion_f_reg,MAX(enf_intervencion_h_reg) enf_intervencion_h_reg FROM hc_enf_intervencion WHERE enf_intervencion_doc='".$datos['enf_intervencion_doc']."' and enf_intervencion_f_reg='".$getfecha."' ";
				$resultado=$this->db->query($existe);
				$resultado=$resultado->row();
				$result=$resultado->hc_enf_intervencion;
				$result2f=$resultado->enf_intervencion_h_reg;
				$hora = date_create($result2f);
            	date_add($hora, date_interval_create_from_date_string("1 hour"));
            	$obhora=date_format($hora,"h:i:s");
				if($result2f)
		...
		function actualizar_historiintervencion_enf($datos){
			...
			$this->db->where("enf_intervencion_h_inicio",$datos['enf_intervencion_h_inicio']);
