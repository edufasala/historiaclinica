<?php
	...
	class Enfermera_model extends CI_Model
	{
		...
		function consultar_estadisticas(){
			...
			$visita="SELECT COUNT(*) id_visita FROM visita ";
			$visita=$this->db->query($visita);
			$visita=$visita->row();
			$visita=$visita->id_visita;

			$hcenf="SELECT COUNT(*) id_hc_enf_pad FROM hc_enf_pad WHERE id_hc_enf_pad!='PADE0000'";
			$hcenf=$this->db->query($hcenf);
			$hcenf=$hcenf->row();
			$hcenf=$hcenf->id_hc_enf_pad;
			...
			$consulta="SELECT COUNT(*) agenda_nombres FROM agenda INNER JOIN hc_enf_pad on hc_enf_pad.enf_pad_f_reg=agenda.agenda_f_cita WHERE agenda_nombres_profesional='".$q."' and agenda_f_cita='".$getfecha."' and agenda_estado_cita='p' ";
