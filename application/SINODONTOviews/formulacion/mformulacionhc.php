<?php
	...
	foreach ($cn['fmhc'] as $fila)
	{
		$obnu=$fila->id;
		$obfechafm=utf8_decode($fila->pad_f_reg);
		$obhorahfm=utf8_decode($fila->pad_h_reg);
		$obfechavfm="";//utf8_decode($fila->pad_f_v);
		$obldig=utf8_decode($fila->pad_medicamentos);//lista
	...
	for ($i=0; $i < count($porciones)-1; $i++)
	{
		...
		else
		{
			...
			$pdf->SetX(11);
			$pdf->Ln(4);
			$pdf->Cell(50,4,'Via Administracion: '.$subporciones[2],1,0,'L');
			// $pdf->Cell(146,4,'Observaciones:'.$subporciones[4],1,0,'L');
			$pdf->MultiCell(146,4,'POSOLOGIA: '.strtolower($subporciones[1].", ".$subporciones[3].", ".$subporciones[6]),1,'J');
			// $pdf->Ln(4);
	...
	$pdf->SetX(10);
	$pdf->Cell(4,4,utf8_decode('N°'),1,0,'L');
	$pdf->Cell(182,4,strtoupper('Medicamento'),1,0,'C');
	// $pdf->SetFont('Arial','',5);
	// $pdf->Cell(90,4,' -    Impreso: '.str_replace('of','de',$GLOBALS['fef']).' '.date('h:i:s a'),0,0,'L');
	$pdf->SetFont('Arial','',6);
	$pdf->Cell(10,4,'Cantidad',1,0,'C');
