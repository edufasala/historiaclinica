<?php
	...
	class Super extends CI_Controller {
		...
		function index(){
			...
			$recibodatos['cn']=$this->super_model->consultar_perfil($this->session->userdata('s_id'));
			$this->load->view('plantilla/header');
			$this->load->view('plantilla/menu_s');
			$this->load->view('perfil/vspadmin',$recibodatos);
		...
		function actualizar(){
			...
			$rs=$this->super_model->actualizar_perfil($datos);
			echo $rs;
		}
		function calcular(){
			$dato=$this->input->post('lafecha');
			$rs=$this->super_model->calcular_edad($dato);
			echo $rs;			
		}
