<?php
	...
	class Admin extends CI_Controller {
		...
		function index(){
			...
			$recibodatos['cn']=$this->admin_model->consultar_perfil($this->session->userdata('s_id'));
			$this->load->view('plantilla/header');
			$this->load->view('plantilla/menu_a');
			$this->load->view('perfil/vadmin',$recibodatos);
		...
		function actualizar(){
			...
			// $datos['img_empleado']=$this->input->post('ftanterior');
			...
			$rs=$this->admin_model->actualizar_perfil($datos);
			echo $rs;
		}
		function calcular(){
			$dato=$this->input->post('lafecha');
			$rs=$this->admin_model->calcular_edad($dato);
			echo $rs;			
		}
