<?php
	...
	class Trabajador_model extends CI_Model
	{
		...
		function registrar_trabajador($datos){
			// $this->input->post('');
			$auxid=$this->session->userdata('s_id');
		...
		function consultar_todos_trabajadores(){
			$this->db->where("tipo_empleado","tra");
			$consulta=$this->db->get("registro_empleado");
		...
		function consultar_todos_medicos(){
			// $this->db->where("tipo_empleado","tra");
			$consulta=$this->db->get("registro_medico");
			return $consulta->result();
			if($consulta->num_rows()>0)
			{
				return $consulta->result();//->row();
		...
		function actualizar_trabajador($datos){
		...
		function eliminar_trabajador($id){
			$this->db->delete('registro_empleado',array('id_empleado'=>$id));
			$this->db->delete('usuarios',array('id_usuario'=>$id));
		}
		function actualizar_medico($datos){
			$aux=$datos['id_medico'];
			...
			$campos= array('nombres' => $datos['nombres'],
				...
				'celular' => $datos['celular'],
				'sexo' => $datos['sexo'],
				'img_medico' => $datos['img_medico'],
				'estado_medico' => $datos['estado_medico']);

			$campos_usuario= array(
						'contra'=>$datos['contra'],
						'estado'=>$datos['estado_medico']);
		
			$this->db->where('id_medico',$aux);
			// $this->db->insert('registro_empleado',$campos);
			$query=$this->db->update('registro_medico',$campos);
		...
		function eliminar_medico($id){
			$this->db->delete('registro_medico',array('id_medico'=>$id));
			$this->db->delete('usuarios',array('id_usuario'=>$id));
		}
		...
		function consultar_trabajador_nombres($busqueda){
			$this->db->where("tipo_empleado","tra");
		...
		function consultar_trabajador_todos(){
			$this->db->where("tipo_empleado","tra");
			$consulta=$this->db->get("registro_empleado");
			return $consulta->result();
			if($consulta->num_rows()>0)
		...
		function consultar_medico_nombres($busqueda){
			// $this->db->where("tipo_empleado","tra");
			$this->db->like("nombres",$busqueda);
			$consulta=$this->db->get("registro_medico");
			return $consulta->result();
		...
		function consultar_medico_todos(){
			// $this->db->where("tipo_empleado","tra");
			$consulta=$this->db->get("registro_medico");
			return $consulta->result();
			if($consulta->num_rows()>0)
		...
		function consultar_separados_trabajador($start,$length,$seach,$direccion,$cual){
			$columna="";
			if($cual==0)
			{
